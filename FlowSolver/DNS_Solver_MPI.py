...
from .FiniteElement import TaylorHood, SplitPV
...
class DNS_IPCS_Solver:
    def __init__(self, mesh, boundary, nu=None, dt=None, path=None, noise=False, sourceterm=None, restart=0.0, order=(2,1)):
        """
        :param mesh:
        :param boundary:
        :param nu:
        ...
        """
        element = SplitPV(mesh = mesh, order=order) # initialise finite element space
...
#        self.Vec_pre = bcpre
    ...
    def solve(self, method='iterative', lusolver='mumps',iterative_num=20,iterative_tol=1e-7,relax_factor=1.0):
        ...
        if self.bcreset == 1:         
            ...
            seems not correct, not reset bcs of A 
            """   
            self.__BCviaMatrix_Mat()
            self.__BCviaMatrix_Vec()
            self.bcreset = 0
...
#            eps_diff=norm(diff, norm_type='l2') 
#            eps_cur=norm(self.u_.vector(), norm_type='L2')
#            eps_ral=eps_diff/eps_cur
#            eps=eps_ral
