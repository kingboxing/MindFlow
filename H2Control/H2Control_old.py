...
from scipy.sparse import csr_matrix, identity, bmat,isspmatrix,isspmatrix_csc,isspmatrix_csr,isspmatrix_coo
#%%
...
class H2OptimalControl:
    ...
    def sensornoise(self,Vn=None):
        ...
        if np.asarray((Vn==None)).any() or np.asarray((Vn==0)).any():
            ...
            if np.size(Vn)==size:
                V_half=np.matrix(np.diag(np.asarray(1.0/Vn).flatten()))
            else:
    ...
    def costfunction(self,Ru=1,beta=1,Qq=None):
        ...
        if Ru==None or Ru==0:
            ...
            if np.size(Ru)==size:
                R_half=np.matrix(np.diag(np.asarray(1.0/Ru).flatten()))
            else:
        ...
        if isspmatrix(Q_half):
            ...
            return Q_half, R_half
    def __h2norm(self,MatA, MatZ, divide=10, form='direct'):
        if form == 'direct':
    ...
    def LQE(self,k0=None,noise=None,disturbance=None,opts=None,Ru=1,beta=1,Qq=None,delta=-0.02,gamma=0.0):
        """
        ...
        if opts is None:
            ...
            opts.adi.output = 1
            opts.nm.output = 1
            opts.adi.paratype = mess.MESS_LRCFADI_PARA_ADAPTIVE_V
        ...
        if k0 is 'Bernoulli': ## if it's 'Bernoulli', then form k0 by solving a Bernoulli equation
            ...
            self.k0=k0*np.matrix(np.ones(self.model.C.shape))
        elif np.any(k0)==None:
            self.k0=None
        elif np.shape(np.matrix(k0))==self.model.C.shape or np.shape(np.matrix(k0))==np.transpose(self.model.C).shape:
            self.k0=np.matrix(k0)
        else:
        ...
        return Z, Status, Kf, Sigma2
    
    def LQR(self,k0=None,Ru=1,beta=1,Qq=None,opts=None,disturbance=None,delta=-0.02,gamma=0.0):
        """
        ...
        if k0 is 'Bernoulli': ## if it's 'Bernoulli', then form k0 by solving a Bernoulli equation
            ...
            self.k0=k0*np.matrix(np.ones(self.model.B.shape))
        elif np.any(k0)==None:
            self.k0=None
        elif np.shape(np.matrix(k0))==self.model.B.shape or np.shape(np.matrix(k0))==np.transpose(self.model.B).shape:
            self.k0=np.matrix(k0)
        else:
            raise ValueError('Invalid initial feedback')
