...
def pymess_riccati_test(model):
    ...
    solver = LQRSolver(model)
    solver.control_penalty(alpha=1)
    solver.measurement(C=model['C'])
...
def pymess_riccati_test2(model):
    ...
    solver.param['riccati_solver']['nm']['k0'] = k0
    solver.param['riccati_solver']['nm']['output'] = 0
    solver.param['riccati_solver']['nm']['res2_tol'] = 1e-5
    solver.param['riccati_solver']['nm']['output'] = True
    solver.param['riccati_solver']['adi']['res2_tol'] = 1e-7
...
def mmess_nmriccati_test(model):
    ...
    k0 = sio.mmread("data/mess/NSE_RE_500_lvl1_Feed0.mtx")
    solver = LQRSolver(model, backend='matlab')
    solver.control_penalty(alpha=1)
    solver.measurement(C=model['C'])
    solver.param['riccati_solver']['nm']['K0'] = k0
    solver.param['riccati_solver']['nm']['info'] = False
    solver.param['riccati_solver']['nm']['inexact'] = 'superlinear'
    solver.param['riccati_solver']['adi']['maxiter'] = 300
...
def mmess_nmriccati_test2(model, mode):
    print(f'------------ MMESS Newton Accumulation Solver ( mode {mode})------------')
    k0 = sio.mmread("data/mess/NSE_RE_500_lvl1_Feed0.mtx")
    solver = LQRSolver(model, backend='matlab')
    solver.control_penalty(alpha=1)
    solver.measurement(C=model['C'])
    ...
    matq = cholesky(m.tocsc(), ordering_method="natural").L().transpose()
    sol = solver.iter_solve(num_iter=3, MatQ=matq, mode=mode)
    norm_pa = sol['sqnorm_sys']
    ...
    print("2nd: it = %d \t rel_res2 = %e\t res2 = %e \n" % (
        sol['status'][2]['niter'], sol['status'][2]['res'][-1], sol['status'][2]['rc'][-1]))


def mmess_radiriccati_test(model, case):
    print(f'------------ MMESS RADI Solver (case {case})------------')
    k0 = sio.mmread("data/mess/NSE_RE_500_lvl1_Feed0.mtx")
    solver = LQRSolver(model, backend='matlab', method='radi')
    if case == 0:
        # only K
        solver.param['riccati_solver']['radi']['K0'] = k0
    if case == 1:
        # Z * inv(Y) * Z'
...
# pymess_riccati_test2(model) # very long time due to sparsity due to feedback
mmess_nmriccati_test(model)
# mmess_nmriccati_test2(model, mode = 0) # very long time due to sparsity due to feedback
mmess_nmriccati_test2(model, mode=1)
