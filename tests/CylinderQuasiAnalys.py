...
solver.solve(k=100, Re=Re, sz=-0.5j)

# compare results
eigs = load_complex('./data/eigen/LMQuasiEigen_bf_newton_cylinder_26k_re080_sz_0-5j.txt')
vals, ind = sort_complex(solver.vals)
Error_vals = np.linalg.norm(np.abs(vals - eigs)[0:50], ord=np.inf)
